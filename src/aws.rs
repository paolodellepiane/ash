...
impl Credential {
    fn is_expired(&self) -> bool {
        if let Ok(exp) = chrono::DateTime::parse_from_rfc3339(&self.expiration) {
...
fn get_env_credentials() -> Option<Credential> {
    match (
        ...
        (Ok(access_key), Ok(secret), Ok(region)) => Some(Credential {
...
fn get_shared_credentials() -> Option<Vec<Credential>> {
    ...
    let mut creds: HashMap<_, _> = Ini::load_from_file(&aws_credentials)
        .expect("Can't load aws credentials")
        .iter()
    ...
    let json = serde_json::to_string(&tosave).expect("Can't serialize cache");
    std::fs::write(Config::cache_path(), json).expect("Can't write cache");
    ...
    let instances = doc
        ...
        .filter_map(|i| {
            ...
            let tag_name = tag_set_items.find(|x| {
                x.children()
                    .any(|x| x.has_tag_name("key") && x.text().unwrap_or_default() == "Name")
            })?;
            let name = tag_name
                .children()
